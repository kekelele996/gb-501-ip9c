...
func SeedDemoData(ctx context.Context, db *gorm.DB) error {
	...
	return db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		batches := []model.ProductionBatch{
			...
			{
				BatchNo:          "B20260820-007",
				Specification:    "无菌注射器泡罩包装 60x120 mm",
				Status:           constants.BatchStatusRework,
				ResponsibleTeam:  "甲班",
				PackagingLineID:  lines[0].ID,
				PlannedQuantity:  9000,
				ProducedQuantity: 9000,
				StartedAt:        &startedEarlier,
				HoldReason:       "首次检验密封失败，返工后重新抽样",
				ReworkRound:      1,
			},
		}
		...
		samples := []model.InspectionSample{
			...
			{
				ProductionBatchID: batches[4].ID,
				SampleCode:        "S-007-R0-END-SEAL",
				SamplingPosition:  "首轮末段",
				InspectionItem:    "热封强度",
				Result:            "fail",
				MeasuredValue:     "1.28 N/15mm",
				AcceptanceRange:   ">= 1.50 N/15mm",
				RetestStatus:      "completed",
				ReworkRound:       0,
				InspectorName:     "系统示例",
				InspectedAt:       &inspectedAt,
				Notes:             "首轮不合格记录，进入第 1 次返工后仅供追溯",
			},
			{
				ProductionBatchID: batches[4].ID,
				SampleCode:        "S-007-R1-START-SEAL",
				SamplingPosition:  "返工后起始段",
				InspectionItem:    "热封强度",
				Result:            "pass",
			...
			{
				ProductionBatchID: batches[4].ID,
				SampleCode:        "S-007-R1-MID-DYE",
				SamplingPosition:  "返工后中段",
				InspectionItem:    "染色渗透",
				Result:            "pending",
				AcceptanceRange:   "封边无连续通道",
				RetestStatus:      "none",
