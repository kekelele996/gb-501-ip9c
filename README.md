...
4. 放行只评估**当前轮次**的样本：首轮为第 0 轮，每进入一次返工，批次的 `reworkRound` 加 1，重新登记的样本自动归入当前轮次。当前轮次没有样本，或存在待检验、待复测、不合格结果时不能放行；旧轮次记录保留在检验明细中仅供追溯，不再参与判断。隔离和返工决定不受检验结果限制。
5. 管理员在「审计记录」按操作者、实体或请求 ID 回溯操作。

首次启动会幂等写入 3 条产线、5 个批次和 9 条检验样本，便于直接验证完整流程；已有业务数据时不会覆盖。
