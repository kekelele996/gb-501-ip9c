...
import { Button, Col, Form, Input, Modal, Radio, Row, Select, Space, Tag, Typography, message } from 'antd'
...
import { formatDateTime, formatReworkRound } from '../utils/format'
...
export function InspectionsPage() {
  ...
  const columns: ColumnsType<InspectionSample> = [
    ...
    { title: '检验轮次', render: (_, row) => row.productionBatch && row.reworkRound === row.productionBatch.reworkCount ? <Tag color="blue">本轮 · {formatReworkRound(row.reworkRound)}</Tag> : <Tag>历史 · {formatReworkRound(row.reworkRound)}</Tag> },
  ...
  return (
    <div className="page-stack">
      ...
      <Modal title="登记检验样本" width={640} open={createOpen} confirmLoading={saving} onOk={() => void create()} onCancel={() => setCreateOpen(false)} okText="登记" cancelText="取消">
        <Form form={createForm} layout="vertical"><Form.Item name="productionBatchId" label="生产批次" rules={[{ required: true }]}><Select showSearch optionFilterProp="label" options={batches.map((batch) => ({ value: batch.id, label: `${batch.batchNo} · ${batch.specification}（登记到${formatReworkRound(batch.reworkCount)}）` }))} /></Form.Item><Row gutter={16}><Col span={12}><Form.Item name="sampleCode" label="样本编号" rules={[{ required: true, min: 3 }]}><Input /></Form.Item></Col><Col span={12}><Form.Item name="samplingPosition" label="抽样位置" rules={[{ required: true }]}><Input placeholder="起始/中段/末段" /></Form.Item></Col></Row><Form.Item name="inspectionItem" label="检验项目" rules={[{ required: true }]}><Input placeholder="密封强度、染色渗透等" /></Form.Item><Form.Item name="acceptanceRange" label="接受范围" rules={[{ required: true }]}><Input placeholder="≥ 1.5 N/15mm" /></Form.Item><Form.Item name="notes" label="备注"><Input.TextArea rows={2} /></Form.Item></Form>
      </Modal>
